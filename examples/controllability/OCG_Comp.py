...
Side-by-side comparison of all strategies from the paper:
  ...
  5. DeePC                  — Hankel-based non-parametric prediction (QP)
  6. RH-OCG (RBF maps)      — receding horizon with RBF A/B
  7. RH-OCG (empirical)     — receding horizon with empirical probing
...
print("[2/7] LTV OCG (reference trajectory)...")
t0 = time.perf_counter()
...
# Observation gap using the linearized free response:
dy = y_f - C @ Phi @ z_0
...
print("[3/7] LTV OCG (RBF maps)...  building maps...", end='', flush=True)
t0 = time.perf_counter()
...
print("[4/7] Empirical OCG (open-loop)...")
t0 = time.perf_counter()
...
# --- STRATEGY 5: DeePC ---
print("[5/7] DeePC ...", end='', flush=True)
t0 = time.perf_counter()

# Define DeePC hyperparameters:
T_ini        = 5
L            = 30
lam_g        = 20.0
lam_h        = 2e3
...
# Loop through each stage (initial, middle, target):
for ic in ICs_d:
    ...
    for k in range(seg):
        ...
        if np.any(np.abs(z_off) > 1e3):
            safe = False
            break

    # Append if it did not explode:
    if safe:
...
def build_hankel(seq, depth):
    ...
    # Extract shape and initialize:
    dim = seq.shape[1]
    Nc = seq.shape[0] - depth + 1
    H = np.zeros((depth * dim, Nc))

    # Loop through each dimension and build:
    for i in range(depth):
        H[i*dim:(i+1)*dim, :] = seq[i:i+Nc].T
    return H

# Build a Hankel Matrix for both the control and output:
H_u_d  = build_hankel(u_d, depth)
H_y_d = build_hankel(y_d, depth)

# Stack past and future controls and outputs:
H_full_raw = np.vstack([H_u_d[:T_ini*m, :], H_y_d[:T_ini*p, :],
                        H_u_d[T_ini*m:, :], H_y_d[T_ini*p:, :]])

# Normalize by column to make condition number more reasonable:
...
# Extract past and future matrices again:
...
def deepc_solve(u_ini_flat, y_ini_flat, y_target):
    ...
    # Introduce soft constraints via scaler lambda:
    hankel_pen = lam_h * (
...
U_plan       = None
u_prev       = np.zeros(m)
u_buf        = np.zeros((T_ini, m))
y_buf        = np.array([C @ z_0] * T_ini)
...
for k in range(T):
    ...
    # Resolve for control when needed:
    if (U_plan is None or k % replan_deepc == 0) and N_rem >= min_hor_deepc:
        U_plan = deepc_solve(u_buf.flatten(), y_buf.flatten(), y_f)

    # Set zeros or pull the first control:
    if U_plan is None or len(U_plan) == 0:
        u_k = np.zeros(m)
    else:
        u_k = U_plan[0]
        U_plan = U_plan[1:]

    # Accumulate control and trim:
...
results['deepc'] = dict(label='DeePC', z=z_deepc, U=np.array(U_hist_deepc), e=e, t=t,
                        color='tab:brown', ls='-.')
print(f"\n   h={e[0]:.4f}m  alpha={e[2]:.3f}°  t={t:.1f}s")

# MPC parameters (shared by strategies 6 and 7):
rate_clip = np.deg2rad(8)
replan  = 5
n_probe = 60
min_hor = 15

# --- STRATEGY 6: RH-OCG (RBF maps) ---
print("[6/7] RH-OCG (RBF maps)...")
t0 = time.perf_counter()

def ocg_solve_inner_rbf(z0, y_goal, N_rem, N_probe=60):
    N_p = min(N_probe, N_rem)
    z_nom_ = simulate(z0, np.zeros((N_p, m)))
    C_o_ = np.zeros((p, N_p * m))
    Phi  = np.eye(n)
    for tau in range(N_p-1, -1, -1):
        Ad_k, Bd_k = eval_AB_rbf(C @ z_nom_[tau])
        C_o_[:, tau*m:(tau+1)*m] = C @ Phi @ Bd_k
        Phi = Phi @ Ad_k
    dy = y_goal - C @ z_nom_[-1]
    W   = C_o_ @ C_o_.T
    U_p = np.clip((C_o_.T @ np.linalg.lstsq(W, dy, rcond=None)[0]).reshape(N_p, m), min_ctrl, max_ctrl)
    U = np.vstack([U_p, np.zeros((N_rem - N_p, m))]) if N_p < N_rem else U_p
    return U, dy, C_o_

z = z_0.copy(); U_hist_rbf = []; U_plan = None; u_prev = np.zeros(m)
C_o_acc_rbf = np.zeros((p, T * m)); C_o_last_rbf = None; k_replan = 0

for k in range(T):
    N_rem = T - k
    if (U_plan is None or k % replan == 0) and N_rem >= min_hor:
        U_plan, delta_y, C_o_last_rbf = ocg_solve_inner_rbf(z, y_f, N_rem, N_probe=n_probe)
        k_replan = k
    if C_o_last_rbf is not None:
        tau_loc = k - k_replan
        if tau_loc * m < C_o_last_rbf.shape[1]:
            C_o_acc_rbf[:, k*m:(k+1)*m] = C_o_last_rbf[:, tau_loc*m:(tau_loc+1)*m]
    if U_plan is None or len(U_plan) == 0:
        u_k = np.zeros(m)
    else:
        u_k = U_plan[0]; U_plan = U_plan[1:]
    u_k    = u_prev + np.clip(u_k - u_prev, -rate_clip, rate_clip)
    u_prev = u_k.copy(); z = rk4(z, u_k); U_hist_rbf.append(u_k.copy())

z_rbf_rh = simulate(z_0, np.array(U_hist_rbf))
t = time.perf_counter() - t0

e = [abs(z_rbf_rh[-1,i]*sc[i] - y_f[i]*sc[i]) for i in range(p)]
results['rbf_rh'] = dict(label='RH-OCG (RBF)', z=z_rbf_rh, U=np.array(U_hist_rbf), e=e, t=t,
                         color='tab:cyan', ls='-')
print(f"   h={e[0]:.4f}m  alpha={e[2]:.3f}°  t={t:.2f}s")


# --- STRATEGY 7: RH-OCG (empirical) ---
print("[7/7] RH-OCG (empirical)...")
